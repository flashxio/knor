...
static void kmeanspp_init(const double* matrix,
        clustercore::prune_clusters::ptr clusters,
 ...
 *	\param cluster_assignments Which cluster each sample falls into.
 */
static void EM_step(const double* matrix, clustercore::prune_clusters::ptr cls,
        unsigned* cluster_assignments, knor::llong_t* cluster_assignment_counts,
        clustercore::thd_safe_bool_vector::ptr recalculated_v,
        std::vector<double>& dist_v,
...
#pragma omp parallel for firstprivate(matrix, pt_cl)\
    ...
    for (unsigned clust_idx = 0; clust_idx < K; clust_idx++) {
        ...
        cls->set_prev_dist(clustercore::eucl_dist(
...
clustercore::cluster_t compute_min_kmeans(const double* matrix, double* clusters_ptr,
        unsigned* cluster_assignments, knor::llong_t* cluster_assignment_counts,
        const size_t num_rows, const size_t num_cols, const unsigned k,
    ...
    clustercore::prune_clusters::ptr clusters =
        clustercore::prune_clusters::create(K, NUM_COLS);
    ...
    // For pruning
    clustercore::thd_safe_bool_vector::ptr recalculated_v =
        clustercore::thd_safe_bool_vector::create(NUM_ROWS, false);
