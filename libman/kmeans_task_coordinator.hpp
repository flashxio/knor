...
class kmeans_task_coordinator : public knor::base {
protected: // So lazy ..
    // Metadata
    // max index stored within each threads partition
    std::shared_ptr<core::prune_clusters> cltrs;
    std::shared_ptr<core::thd_safe_bool_vector> recalculated_v;
...
public:
    ...
    std::shared_ptr<core::prune_clusters> get_gcltrs() {
        return cltrs;
    }
    ...
    core::cluster_t mb_run(double* allocd_data=NULL);
